...
He indicado las lineas justas para comprobar que el repositorio GSE46687 **no aporta información adicional**. Podría parecer que **gender** en el pData sería un dato importante pero todas las muestras son de mujeres como ya hemos explicado al inicio del estudio. Todo el resto de campos son idénticos. Las fechas de las muestras son todas las mismas.
...
#### **Una mirada rápida con summary()**

<br>

Aunque los datos viene de archivos CEL de un reprositorio publicado, he preferido dar una mirada directa a los datos mediante la función clásica **summary()** que nos permite ver que no hay datos NA, ni datos NULL y que los máximos, mínimos y medias se mueven todos entre márgenes razonables.
...
En esta ocasión podemos hacer más caso a la función ArrayQualityMetrics pero por ejemplo vemos que de los tres métodos de control solo el análisis de DistanceBetweenArrays nos da un **posible oulier** en la muestra etiquetada como **Xm_rep1**, la muestra con síndrome de Turner con cromosoma de origen materno 45Xm. En principio, a menos que aparezca alguna otro dato, no le daremos mucha importancia.
...
Al aparecer nuevamente esta muestra como posible outlier, me dispongo a realizar un **estudio provisional sin la muestra Xmrep1** y comprobar el mismo gráfico PCA a ver si se notan los cambios.
...
Podemos hacer algunas pruebas variando el porcentaje de genes que se expresan de forma clara. En concreto visualizamos el 75% y los datos de otros porcentajes:
...
En nuestro caso nos quedaremos con un subset de probes correspondiente al **25%** de "genes" expresados. Pongo "genes" entrecomillados porque he estado usando este termino con abuso de lenguaje, ya que son tránscritos de RNAm que combinan distintos exones del mismo gen. Por lo tanto hemos eliminado provisionalmente el 95% de los probesets aparentemente sin valor en el estudio.
...
En este apartado vamos a construir la matriz de contrastes. En nuestro caso vamos a estudiar dos comparaciones:
...
cont.matrix <- makeContrasts(Xp_vs_Xm = Xp-Xm, Xm_vs_XX = Xm-XX,levels=designMat)
print(cont.matrix)
```
...
Analizando los resultados del gráfico Volcanplot, nos sorprende ver que hay un transcrito 224588_at que provoca un desplazamiento de todo el mapa de expresión. 
...
Al tratarse de un transcrito inactivo, me inclino a pensar que hay algun tipo de anomalía y decido apartarlo y ver qué apariencia tiene el mismo gráfico sin dicho probeset. Creamos el nuevo expressionSet sin el probe set **224588_at**. Visualizo los datos de la posición y de las dimensiones del nuevo probeset:
...
volcanoplot(fit.main, coef=1, highlight=4, names=rownames(fit.main), 
             main=paste("Differentially expressed genes", colnames(cont.matrix)[1], sep="\n"))
abline(v=c(-1,1))
volcanoplot(fit.main, coef=2, highlight=4, names=rownames(fit.main), 
             main=paste("Differentially expressed genes", colnames(cont.matrix)[2], sep="\n"))
abline(v=c(-1,1))
#volcanoplot(fit.main, coef=3, highlight=4, names=rownames(fit.main), 
...
title("Genes in common between the two comparisons\n Genes selected with FDR < 0.1 and logFC > 1")
...
#top_Xp_XX <- topTable (fit.main, number=nrow(fit.main), coef="Xp_vs_XX", adjust="fdr")  
#head(top_Xp_XX)
...
#topAnnotated_Xp_vs_XX <- annotatedTopTable(top_Xp_XX,
#anotPackage="hgu133plus2.db")
...
#write.csv(topAnnotated_Xp_vs_XX, file="./results/topAnnotated_Xp_vs_XX.csv")
write.csv(topAnnotated_Xm_vs_XX, file="./results/topAnnotated_Xm_vs_XX.csv")

head(topAnnotated_Xp_vs_Xm,10)
#head(topAnnotated_Xp_vs_XX,10)
...
whichGenes<-topTab["adj.P.Val"]<0.30
...
```{r grafico enrichement}

cnetplot(enrich.result, categorySize = "geneNum", schowCategory = 15, 
        vertex.label.cex = 0.75)
#```

barplot(enrich.result, showCategory=8)

dotplot(enrich.result, showCategory=15)

#emapplot(enrich.result)
...
#### **problemas que no me han permitido llegar a las conclusiones finales**


Aunque he dedicado muchos muchos esfuerzos y muchas muchas horas, no he sido capaz  de resolver algunas dudas. Las cito en los siguientes puntos:

- **Relación entre Xp y XX: **No acabo de entender que existan genes diferenciados entre Xm y XX , también entre Xm y Xp pero en cambio no existan diferencias entre Xp y XX. Biológicamente no me lo explico e incluso matemáticamente tampoco.

- **Función enrichPathway: ** la función enrichPathway junto a la función Reactome me ha dado problemas indicando que **"No Gene can be mapped"** que entiendo que significa que no detecta genes o que no están bien identificados "Expected input gene ID: 7298,7965,129807.." No he sido capaz de solventarlo. No obstante, de las 3 comparaciones que he realizado, en el estudio me aparecen 2 tránscritos en la primera comparación, nada en la segunda (no había genes diferenciados) y nada en la tercera. He probado varios valores de cutoff de la función pero nada de nada.

- **cnetplot: **Al no conseguir crear el mappeado anterior, no he conseguido tener el gráfico relacional entre "genes".
  
  
Por lo tanto mis conclusiones las he ido citando en cada apartado pero no puedo llegar a las conclsuiones finales por falta de resultados.
